# 📤 PF1 Quote Column Extractor - TEST VERSION (10 samples)
# Quick test script for processing 10 PDFs to validate extraction quality
...
# STEP 3: Process first 10 PDFs in PF1 folder
def process_test_pdfs():
    """Process first 10 PDF files in PF1 directory for testing"""
    ...
    print(f"🧪 TEST MODE: Processing only first {max_files} files...")
    
    for filepath in pdf_files:
        processed_files += 1
        print(f"📄 Processing {processed_files}/{min(max_files, total_files)}: {os.path.basename(filepath)}...")
        ...
        print(f"✅ Extracted {len(extracted)} fields from {os.path.basename(filepath)}")
        
        # Stop after max_files
        if processed_files >= max_files:
            print(f"🛑 Test completed: Processed {max_files} files")
            break
    
    return field_values

# STEP 4: Generate test output
def generate_test_output(field_values):
    """Generate CSV and console output for test run"""
    print(f"\n🧪 TEST RESULTS: Found {len(field_values)} new potential Airtable columns:")
    print("=" * 70)
    
    # Console output
    for field, values in sorted(field_values.items()):
        sample_values = sorted(list(values))[:3]  # Show first 3 values for test
        print(f"• {field}: {sample_values}")
    
    # CSV output with detailed values
    csv_filename = "column_suggestions_test.csv"
    ...
    if field_values:
        print(f"\n🏆 Top 5 most common fields in test:")
...
if __name__ == "__main__":
    print("🧪 PF1 Quote Column Extractor - TEST MODE")
    print("=" * 50)
    print("Processing first 10 PDFs for validation...")
    
    # Process test PDFs
    field_values = process_test_pdfs()
    
    # Generate test output
    if field_values:
        generate_test_output(field_values)
    else:
        print("❌ No new fields found in test run")
    
    print("\n✅ Test extraction complete!")
    print("💡 Run pf1_quote_extractor_full.py for complete dataset processing") 